...
1. 确保将 `pubspec.yaml` 中的 `version` 字段递增（例如从 `0.1.0` 改为 `0.1.1`），然后提交代码到 `main` 分支。
...
4. 等待几分钟，工作流会自动在仓库的 **Releases** 页面创建一个正式 Release，并将构建好的、针对 arm64 架构瘦身的 APK 文件（如 `PubMedMobile-v0.1.1-arm64.apk`）作为附件上传，所有人即可公开下载。

> **提示**：这个自动构建出来的 APK 和你在本地执行 `flutter build apk --release --target-platform=android-arm64` 产生的产物是一致的。
